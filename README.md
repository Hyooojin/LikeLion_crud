...
멋쟁이 사자처럼에서 배운 것을 나름대로 단계별로 정리하는 것을 목표로한다.
...
1. data를 이동시키기 위해서 controller를 작성한다. 
...
값을 저장할 수 있는 Database가 필요하다. Database를 사용하기 위해서는 MVC패턴 중에 model의 개념을 이용한다. 여러명의 사용자에게 

값을 입력받고 그 값을 database에 저장하여 필요할 때 database에서 꺼내서 사용할 수 있도록 한다.
...
1. 준비된 테이블을 실제로 생성시킨다.

    $ rake db:migrate

- db > schema.rb를 확인

1. 넘어오는 값들을 database에 저장시킨다.
...
1. routes.rb에 회원가입과 로그인에 쓰일 것들을 라우팅, method정의, view를 만들어준다.
...
sign_up.erb와 sign_up_process.erb

1. 회원가입을 통해 user값을 입력받고, db에 저장시킨다. 
...
로그인을 하면 질문을 쓸 수 있고, 익명을 선택한 사람은 이름을 보여주지 말고, 
...
@msg를 볼 수 없지만 나중에 flash를 이용하면 메세지를 띄울 수 있다.
...
1. login 후에 session에 저장된 값으로 user 정보를 출력한다. 
...
1. 수정을 하기 위해서 url에 게시물의 해당id값을 함께 넘겨준다.
